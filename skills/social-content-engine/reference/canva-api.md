...
`title` にアセット種別のキーワード（例: 「正方形投稿」「ストーリー」「メールヘッダー」）が含まれるもので絞り込む。
...
POST /autofills
{
  ...
  "title": "梅雨前点検キャンペーン — 投稿1",
  "data": {
    "Headline": { "type": "text", "text": "梅雨前のエアコン点検、6月の枠を受付中" },
...
チラシなど印刷物は `"type": "pdf"` でエクスポートし、印刷会社への入稿用に
渡す（印刷の推奨は 350dpi・塗り足し 3mm。テンプレート側の設定で決まる）。
